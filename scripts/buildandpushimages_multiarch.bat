...
:DEFAULT_REGISTRY
    ...
    SET REGISTRY_NAMESPACE=eap-eks

:MAIN
:: Uncomment the below line if you want to enable login before pushing
:: docker login %REGISTRY_URL%

echo "building and pushing image helloworld"
pushd source\eap-docker\eap-docker
docker buildx build --platform ${PLATFORMS} -f Dockerfile --push --tag ${REGISTRY_URL}/${REGISTRY_NAMESPACE}/helloworld .
popd
